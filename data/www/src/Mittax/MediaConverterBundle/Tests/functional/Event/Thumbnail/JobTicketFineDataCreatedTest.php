 ...
 */
class JobTicketFineDataCreatedTest extends AbstractKernelTestCase
{
    ...
    {
        $event = new JobTicketFineDataCreated($this->_jobTicketFixure);

        $this->assertInstanceOf(ITicket::class, $event->getJobTicket());

        $event = Dispatcher::getInstance()->dispatch('thumbnail.jobticket.finedata.created', $event);
